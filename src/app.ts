...
async function processNextInput() {
    ...
    try {
        ...
        if (payload.prompt && payload.prompt.template) {
            ...
            const template = config.prompts?.[templateName];
            if (!template) throw new Error(`Unknown prompt template: ${templateName}`);
            promptText = template.prompt;
            responseFormat = template.response_format;
            // template may specify model override
        }
        ...
        const response = await aiService.sendFilesAndPrompt(aiName, tempFiles, promptText, responseFormat, payload.prompt?.model);
        ...
        mqttService.publish(`${config.mqtt.basetopic}/OUTPUT`, JSON.stringify(out), false);
...
function extractTextFromAiResponse(resp: any): string {
    try {
        if (!resp) return '';
        if (resp.choices && resp.choices.length > 0) return resp.choices[0].message.content || '';
